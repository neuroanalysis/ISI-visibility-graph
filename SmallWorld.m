...
function Properties = SmallWorld(NetworkName, Adjacency)
    ...
    text(max(x)/2,max(y)*0.8,['\gamma=' num2str(-slope,'%0.1f')],...
        'FontSize',14,'FontWeight','bold')
    text(max(x)/2,max(y)*0.6,['R^2=' num2str(R2,'%0.3f')],...
        'FontSize',14,'FontWeight','bold')
    title('Degree Distribution'); xlabel('Links'); ylabel('Count of neurons')    
